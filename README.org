...
- Type `@` inside a Codex terminal to open a Files context picker
...
| @ (in Codex)  | Open Files context picker    |
...
Inside the Codex terminal, typing =@= opens a small context menu with a =Files= picker. The picker searches workspace files by name, path, or extension, skips common ignored/generated paths, and inserts relative file mentions such as =@src/components/Button.tsx= directly into the Codex prompt.
